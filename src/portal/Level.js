...
const matMetal = () =>
  new THREE.MeshStandardMaterial({ color: 0x24272f, roughness: 0.7, metalness: 0.25 });
const matPanel = () =>
...
function chamber0(ctx) {
  ...
  addBox(ctx, V(-6.5, 0, -4), V(-6, 6, 26), true); // SOL DUVAR (portallanabilir)
  addBox(ctx, V(6, 0, -4), V(6.5, 6, 26), false); // sağ duvar
  addBox(ctx, V(-6, 0, -4.5), V(6, 6, -4), false); // arka duvar
  addBox(ctx, V(-6, 0, 26), V(6, 6, 26.5), false); // ön duvar
  ctx.spawn = V(0, 0.1, -1);
...
function chamber1(ctx) {
  ...
  addBox(ctx, V(-6.5, 0, -4), V(-6, 6, 6), true); // sol duvar portallanabilir (başlangıç tarafı)
  addBox(ctx, V(6, 0, -4), V(6.5, 6, 26), false);
  addBox(ctx, V(-6, 0, -4.5), V(6, 6, -4), false);
  addBox(ctx, V(-6, 0, 26), V(6, 6, 26.5), false);
  ctx.spawn = V(0, 0.1, -1);
  exitPad(ctx, V(0, 0.15, 22));
...
function chamber2(ctx) {
  // büyük zemin
  addBox(ctx, V(-14, -0.5, -14), V(14, 0, 14), true);
  // çevre duvarlar (portallanabilir)
  addBox(ctx, V(-14.5, 0, -14), V(-14, 8, 14), true);
  addBox(ctx, V(14, 0, -14), V(14.5, 8, 14), true);
  addBox(ctx, V(-14, 0, -14.5), V(14, 8, -14), true);
  addBox(ctx, V(-14, 0, 14), V(14, 8, 14.5), true);
  // derin kuyu (fırlatma için) — zeminde delik yerine ayrı bir çukur platformu
  ...
  // kuyu ağzını açmak için zemini delik etrafında parçalı kur:
  // (yukarıdaki büyük zemin deliği kapatıyor; basitlik için kuyuyu zemin üstünde bırakıyoruz)
  // yüksek kule + üstünde çıkış
  addBox(ctx, V(8, 0, 8), V(12, 7, 12), true); // kule
  ctx.spawn = V(0, 0.1, -8);
